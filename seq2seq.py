...
def train(train_set, val_set, test_set, tokenizer, model, model_name = 'facebook/bart-base', max_source_length = 64, max_target_length = 8, epochs = 40, batch_size = 4):
    ...
    tokenized_inputs = concatenate_datasets([train_set, test_set]).map(lambda x: tokenizer(x["source"], truncation=True), batched=True, remove_columns=['wikidata_id', 'source', 'target'])
    print(f"Max source length: {max_source_length}")

    # The maximum total sequence length for target text after tokenization. 
    # Sequences longer than this will be truncated, sequences shorter will be padded."
    tokenized_targets = concatenate_datasets([train_set, test_set]).map(lambda x: tokenizer(x["target"], truncation=True), batched=True, remove_columns=['wikidata_id', 'source', 'target'])
    #max_target_length = max([len(str(x)) for x in tokenized_targets["input_ids"]])

    print('Tokenized targets: ', tokenized_targets)
    print(f"Max target length: {max_target_length}")

    tokenized_train_dataset = train_set.map(preprocess_function, batched=True, remove_columns=['wikidata_id', 'source', 'target'])
    print(f"Keys of tokenized dataset: {list(tokenized_train_dataset.features)}")

    tokenized_val_dataset = val_set.map(preprocess_function, batched=True, remove_columns=['wikidata_id', 'source', 'target'])
...
def test(dataset, model_name, model, tokenizer, input_file = 'dataset/test.json', \
                     batch_size = 4, max_len = 32, min_len = 1):
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        texts = [item['source'] for item in subset]
    ...
    label_list = [item['target'] for item in dataset]
...
# python seq2seq.py --mode "test" --model_name "Helsinki-NLP/opus-mt-en-vi" --model_path "opus-mt-en-vi\checkpoint-63" --test_path "dataset/test.json" --test_batch_size 4 --max_source_length 32 --min_target_length 1 --source_prefix ""
