...
def generate_dataset(dataset, model_name, model, tokenizer, use_force_words = False, input_file = 'dataset/test_random.json', \
    ...
    #print('pred_list: ', len(pred_list),  pred_list[0])
...
def add_target(current_list, source, target, pred, source_column = 'source', target_column = 'target'):
    ...
    if (flag == False):
        current_list.append({source_column: source, target_column: pred, 'old_' + target_column: target})
...
def create_train_set(opt_d2t_pred_list, opt_t2d_pred_list, train_list, current_list = [], \
    ...
    for opt_d2t_pred, opt_t2d_pred, item in zip(opt_d2t_pred_list, opt_t2d_pred_list, train_list):
        ...
        # value order (too strict condition)
        ...
        # repetitions (too strict condition)
        '''con3 = False
        if (compute_repetition_single(opt_d2t_pred) < 0.02):
            con3 = True'''
...
def merge_data(list1, list2, source_column = '', target_column = ''):
    ...
    elif (len(list1) < len(list2)): list1 += list2[0:len(list2)-len(list1)] # may have repetitive data
    
    return list1


def get_self_memory(d2t_train_list, model_name, model, t2d_model, tokenizer, use_force_words = False, \
                    decoding_type = 'greedy', num_beam = 4, batch_size = 4, test_batch_size = 16, max_len = 256, \
                    min_len = 4, source_column = 'source', target_column = 'target', dataset_name = 'wida2wl', \
                    merge_new_data = True, same_data_size = True, source_prefix = '', self_mem_percent = 10, \
                    repeat_self_mem = True):

    self_mem_percent = self_mem_percent/100
    
    # create BM25 model
    corpus = [item['source'] for item in d2t_train_list]
    tokenized_corpus = [doc.split(" ") for doc in corpus]
    bm25 = BM25Okapi(tokenized_corpus)
    
    d2t_train_list_no_prefix = [{source_column: remove_prefix(item[source_column], source_prefix), \
                                target_column: item[target_column]} for item in d2t_train_list]
    print('d2t_train_list_no_prefix: ', d2t_train_list_no_prefix[0], len(d2t_train_list_no_prefix))
    print('------------')
    
    #random.shuffle(d2t_train_list)
    sub_set = d2t_train_list[0:int(len(d2t_train_list)*self_mem_percent)] # get % data
    sub_set_no_prefix = [{source_column: remove_prefix(item[source_column], source_prefix), \
                            target_column: item[target_column]} for item in sub_set]
    
    print('sub_set: ', sub_set[0], len(sub_set))
    ...
    while(True):
        ...
        #if (i > 20): break # avoid to repeat so many times
        print('---------------------------------------------')
        print('-- Loop: ', i)
        
        visited_sources += [item[source_column] for item in sub_set]
        
        # data to text predictions
        d2t_pred_list = generate_dataset(sub_set, model_name, model, tokenizer, use_force_words = use_force_words, \
        ...
        print('d2t_pred_list: ', len(d2t_pred_list), d2t_pred_list[0])
        print('------------')
        ...
        t2d_pred_list = generate_dataset(t2d_train_list, model_name, t2d_model, tokenizer, use_force_words = use_force_words, \
                                         decoding_type = decoding_type, num_beam = num_beam, batch_size = test_batch_size, \
                                         max_len = max_len, min_len = min_len, source_column = source_column, source_prefix = source_prefix)
        print('t2d_pred_list: ', len(t2d_pred_list), t2d_pred_list[0])
        print('------------')
        
        # optimize data to text predictions
        opt_d2t_pred_list = [[optimize_target(item[source_column], pred[0])] for pred, item in zip(d2t_pred_list, sub_set_no_prefix)]  
        print('opt_d2t_pred_list: ', len(opt_d2t_pred_list), opt_d2t_pred_list[0])
        print('------------')
        
        opt_train_list = [{source_column:source_prefix + pred[0]} for pred in opt_d2t_pred_list]
        print('opt_train_list: ', len(opt_train_list), opt_train_list[0])
        print('------------')
        ...
        opt_t2d_pred_list = generate_dataset(opt_train_list, model_name, t2d_model, tokenizer, use_force_words = use_force_words, \
                                             decoding_type = decoding_type, num_beam = num_beam, batch_size = test_batch_size, \
                                             max_len = max_len, min_len = min_len, source_column = source_column, source_prefix = source_prefix)
        print('opt_t2d_pred_list: ', len(opt_t2d_pred_list), opt_t2d_pred_list[0])
        print('------------')
        
        # hold previous variables
        previous_len = len(d2t_current_list)
        previous_list = d2t_current_list.copy()
        
        # create new d2t train set
        d2t_current_list = create_train_set(opt_d2t_pred_list, opt_t2d_pred_list, \
                                            sub_set_no_prefix, d2t_current_list, source_column = source_column, \
                                            target_column = target_column, dataset_name = dataset_name)
        
        # assume maximum items in the first loop
        if (i == 1):
            max_pred_len = int((len(d2t_current_list)/len(sub_set))*len(d2t_train_list))
        print('sub_set: ', len(sub_set))
        print('max_pred_len: ', max_pred_len)
        print('d2t_train_list: ', len(d2t_train_list))
        print('d2t_current_list: ', len(d2t_current_list), d2t_current_list[0])
        print('------------')
        
        # break conditions
        if (len(d2t_current_list) == 0): break # first loop
        if (len(d2t_current_list) == previous_len): break
        if (len(d2t_current_list) > max_pred_len): break
        
        # get new list
        new_list = []
        if (i == 1):
            new_list = list(set([i[source_column] for i in d2t_current_list]))
        else:
            new_list = list(set([i[source_column] for i in d2t_current_list]) - set([i[source_column] for i in previous_list]))
        #print('new_list: ', len(new_list))
        
        # create similar_list by BM25
        similar_list = []
        for source in new_list:
            tokenized_source = source.split(" ")
            #doc_scores = bm25.get_scores(tokenized_source)
            top_sources = bm25.get_top_n(tokenized_source, corpus, n=50)
            for t in top_sources:
                if (t not in similar_list and t not in visited_sources): 
                    similar_list.append(t)
        if (len(similar_list) == 0): break
        similar_list = [{source_column: s, target_column: ''} for s in similar_list]
        for item1 in similar_list:
            for item2 in d2t_train_list:
                if (item1[source_column] == item2[source_column]):
                    item1[target_column] = item2[target_column]
                    break
        print('similar_list: ', len(similar_list), similar_list[0])
        print('------------')
        
        # create sub_set
        sub_set = similar_list.copy()
        sub_set_no_prefix = [{source_column: remove_prefix(item[source_column], source_prefix), \
                            target_column: item[target_column]} for item in sub_set]   
        i = i + 1
        
    # end conditions
    if (len(d2t_current_list) == 0): return []
    if (len(d2t_current_list)//batch_size < 2): return []

    # repeat data to increase memorization, "A Survey on Data Selection for Language Models"
    
    if (repeat_self_mem == True):
        while(len(d2t_current_list) < len(d2t_train_list)*self_mem_percent):
            d2t_current_list += d2t_current_list
    
    if (len(d2t_current_list) > len(d2t_train_list)*self_mem_percent):
        d2t_current_list = d2t_current_list[0:int(len(d2t_train_list)*self_mem_percent)]
    
    print('final d2t_current_list: ', len(d2t_current_list), len(d2t_current_list)/len(d2t_train_list_no_prefix), d2t_current_list[0])
    print('------------')
    
    # save optimized data for extended training
    save_optimized_data(d2t_current_list, dataset_name)

    # take out old targets + predictions from previous inferences
    ...
    if (merge_new_data == True):     
        if (same_data_size == True):
            d2t_current_list = merge_data(d2t_current_list, d2t_train_list_no_prefix, source_column = source_column, \
                                            target_column = target_column)
        else:
            d2t_current_list += d2t_train_list_no_prefix
    
    return d2t_current_list
...
def self_train(model_name, model, tokenizer, data, use_force_words = False, use_fuse_loss = False, \
               ...
               no_self_mem = False, same_data_type = 1, same_data_size = True, source_prefix = '', self_mem_percent = 0.1, repeat_self_mem = True):
    ...
    if (train_percent == 100):
        ...
        n_examples =  int((train_percent/100)*len(train_data2['train']))
        print('n_examples: ', n_examples)
        train_sub_data2 = {'train': datasets.Dataset.from_dict(train_data2['train'][0:n_examples])}
    ...
    if (train_percent == 100):
        ...
        n_examples =  int((train_percent/100)*len(train_data1['train']))
        print('n_examples: ', n_examples)
        train_sub_data1 = {'train': datasets.Dataset.from_dict(train_data1['train'][0:n_examples])}
    ...
    for i in range(1, self_train_epochs + 1): 
        ...
        if (same_data == True):
            # start with the second data part, the first part is for training T2D and D2T models in the beginning
            d2t_train_list1 = d2t_train_list[i*n_examples:(i+1)*n_examples]  
        else:
            d2t_train_list1 = random.sample(d2t_train_list, n_examples)

        
        d2t_current_list = get_self_memory(d2t_train_list1, model_name, model, t2d_model, tokenizer, use_force_words = use_force_words, \
                    decoding_type = decoding_type, num_beam = num_beam, batch_size = test_batch_size, test_batch_size = test_batch_size, \
                    max_len = max_len, min_len = min_len, source_column = source_column, target_column = target_column, \
                    dataset_name = dataset_name, merge_new_data = merge_new_data, same_data_size = same_data_size, \
                    source_prefix = source_prefix, self_mem_percent = self_mem_percent, repeat_self_mem = repeat_self_mem)

        if (len(d2t_current_list) == 0):
            print('There has no or not enough self-memory data to train!')
            return
...
def main(args):
    ...
    if (args.mode == 'train'):
        ...
        repeat_self_mem = (args.repeat_self_mem == 1)
        ...
        self_train(args.model_name, model, tokenizer, data, use_force_words = use_force_words, \
                   ...
                   self_mem_percent = args.self_mem_percent, repeat_self_mem = repeat_self_mem)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--self_mem_percent', type=float, default=10)
    parser.add_argument('--repeat_self_mem', type=int, default=1)
